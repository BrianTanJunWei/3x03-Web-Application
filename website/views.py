...
bcrypt = Bcrypt()
staff = StaffAccounts(email_address=email, name=name)
...
def home():
    ...
    if current_user.is_authenticated:
        ...
        if account_status in (0, 1):
            return render_template("staff_catalog.html", user=current_user, account_status=account_status,
                                   products=products)
        elif account_status == 2:
            return render_template("customer_catalog.html", user=current_user, account_status=account_status,
                                   products=products)
    else:
...
def order():
    ...
    if account_status == 1:
        ...
        return render_template("all_orders.html", user=current_user, orders=orders,
                               customer_first_names=customer_first_names, account_status=account_status,
...
def order_details(order_id):
    ...
    if order:
        ...
        return render_template('order_details.html', user=current_user, order=order,
                               order_items_with_products=order_items_with_products, order_items=order_items,
                               total_cost=total_cost)
    else:
...
def cart():
    ...
    return render_template("cart.html", user=current_user, cart_items_with_products=cart_items_with_products,
                           total_cost=total_cost, account_status=account_status)
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        return render_template("confirmation.html", user=current_user,
                               cart_items_with_products=cart_items_with_products, total_cost=total_cost)

    return render_template('checkout.html', user=current_user, cart_items_with_products=cart_items_with_products,
                           total_cost=total_cost)
...
def mark_order_as_paid(user, total_cost):
    ...
    if account_status == 2:
        ...
        for cart_item in cart_items:
            ...
            order_item = OrderItem(order_id=new_order.order_id, product_id=cart_item.product_id,
...
def generate_order_pdf(user, cart_items, total_cost):
    ...
    for cart_item in cart_items:
        ...
        order_data.append(
            [product.name, cart_item.quantity, f"${product.price:.2f}", f"${cart_item.quantity * product.price:.2f}"])
        ...
        if request.method == 'POST':
            ...
            if user:
                ...
                user.password = bcrypt.generate_password_hash(
                    new_password)  # You need to implement a password hashing function
                db.session.commit()
...
def request_password_reset():
    if request.method == 'POST':
        ...
        if user:
            ...
            log_entry = Logs(
                log_level='INFO',
                log_type='Request reset',
                entity='User',
                log_desc='User request for password reset',
                log_time=datetime.now(),
...
def view_staff(staff_id):
    ...
    if (user_type != 0):
        ...
        if (staffinfo is None):
            valid = 0
        return render_template("admin_staff_details.html", user=current_user, staff=staff, staffinfo=staffinfo,
                               valid=valid, account_status=account_status)
